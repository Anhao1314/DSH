...
| M3 三栏工作台 | ⚠️ 5/6 | ② 待补验（上游额度已于 2026-09-11 恢复，§3.1） |
| M4 系统能力 | ⚠️ 3.5/6 | ②④ 与 ①后半同为待补验（额度已恢复，§3.1）；另修掉一处 M4 引入的「关窗后仍在轮询」回归 |
| M5 打磨与发布 | ✅ 4/4 | 冷启动与 1 小时数据见 §2.1 / §2.2；release 走到签名前一步（本机无证书） |

**一句话**：链路、界面、系统能力全部落地并实测；三条「必须有任务在跑」的验收项曾因**上游额度为 0** 无法执行，该阻塞已于 2026-09-11 解除（换 key + 切官方端点 + 修掉委派 deny 名单的命名空间 bug，见 NOTES §15），现在只差 GUI 操作补验，步骤见 §3.1。
...
| ② | 运行中「停止任务」中止且容器不重启 | ⚠️ 待补验（额度已恢复；现已能跑出运行中会话）。已验证部分：空闲置灰、`cancel` 对空闲会话 502 → 文案「该任务已不在运行」 |
...
| # | 验收项 | 结果 |
| --- | --- | --- |
| ① | 运行中关窗 → 任务继续、菜单栏恢复 | ⚠️ 后半（任务继续）待补验（额度已恢复）。关窗 = `orderOut`，在屏窗口 1→0、菜单栏/Dock 都能叫回；「关窗即停表」在 M5 复测时发现 M4 引入回归（`MenuBarExtra` 的常驻状态栏窗口让 `NSApp.windows` 永远判定可见），已改为按主窗判定并复测：隐藏 0 条/30s、App 进程 0.2–0.4%，叫回后 8s 内 4 次轮询 |
| ② | 运行中 ⌘Q 三按钮 | ⚠️ 待补验（额度已恢复）；代码路径：`applicationShouldTerminate` 三按钮 + `.terminateLater` + 运行态翻转回调 |
| ③ | 空闲 ⌘Q 直接退出、容器被停 | ✅ 1 秒内进程消失，`dsh Exited (143)` |
| ④ | 前台不弹通知、后台完成才弹 | ⚠️ 待补验（额度已恢复）；实现：`guard !Notifier.isUserWatching` + 首任务请求一次授权 |
...
### 3.1 上游额度已恢复（2026-09-11 换 key）→ 三条验收项待补验

当时链路完全正常（`session/create` → `session/prompt` 均 `{"accepted":true}`），失败在模型侧：
投影立刻显示 `sessionStats:{turns:1,steps:1,llmMs:0}`、`tokenUsage.totals` 全 0、`turnOutline.turns[0].response:""`；
抓到的是上游 `402 用户额度不足 … insufficient_quota`。

**2026-09-11 已解除**：新 key 走 DeepSeek 官方端点（TokenDance 网关对新 key 返 `401 密钥不存在`，旧 key 仍是 `402`），
路由切到 `deepseek/deepseek-flash`，真机任务已跑通（NOTES §15.3–15.4：委派出子会话并落盘产物）。
下表四项现在**只差 GUI 操作**，原因不再是额度。
